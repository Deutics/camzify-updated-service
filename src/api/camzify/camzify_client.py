...
class CamzifyClient:
    ...
    async def fetch_all_features(
        ...
        unified_configuration = self._aggregate_streams_and_rules(all_configs)
        # logger.info(f" Loaded {len(streams)} streams and {len(rules)} rule sets.")
        # import pprint
        # pprint.pprint(unified_configuration,indent=4)
        # exit()
    ...
    def _aggregate_streams_and_rules(
        ...
        for cfg in all_configs:
            ...
            # Initialize top-level entry if missing
            if stream_id not in unified:
                unified[stream_id] = {
                    "stream_id": stream_id,
                    "rtsp_url": cfg.get("rtsp_stream_url"),
                    "https_url": cfg.get("https_stream_url"),
                    "camera_width": int(cfg.get("camera_width") or self.default_width),
                    "camera_height": int(cfg.get("camera_height") or self.default_height),
                    "features": {}  # <-- key structural change
                }

            # Update resolution if a higher one is available
            width = int(cfg.get("camera_width") or self.default_width)
            height = int(cfg.get("camera_height") or self.default_height)
            existing = unified[stream_id]
            if (width * height) > (existing["camera_width"] * existing["camera_height"]):
                existing["camera_width"] = width
                existing["camera_height"] = height

            # Attach feature under the correct feature_type
            feature_type = cfg.get("feature_type")
            if not feature_type:
                continue
            ...
            # Allow multiple configs per feature (rare, but supported)
            unified[stream_id]["features"].setdefault(feature_type, []).append(feature_entry)
