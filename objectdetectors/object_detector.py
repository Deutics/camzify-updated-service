...
class ObjectDetector:
    ...
    def _resolve_loader_spec(self) -> tuple[str, str, str]:
        """
        ...
        module = reg["module"]
        klass = reg["class"]
        weights = self.weights_path or reg.get("default_weights", "")

        if not weights:
            raise ValueError(f"No weights_path provided and no default_weights for '{self.model_dir}'")

        return module, klass, weights

    def start(self) -> bool:
        """Dynamically import loader, construct, and load model."""
        try:
            module_path, class_name, weights_path = self._resolve_loader_spec()
